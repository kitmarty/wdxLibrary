...
test.outLine({PAR_ALIGN:wdxConstants.FM_ALIGN_CENTER});
test.outText("Welcome to wdxLibrary! Enjoy it!",{RUN_BOLD:true});
test.outLine({PAR_ALIGN:wdxConstants.FM_ALIGN_CENTER,PAR_SPACE_AFTER:0});
...
test.outLine();
test.outText("Single underline ",{RUN_UNDERLINE:wdxConstants.FM_UL_SINGLE});
test.outText("Wavy heavy underline",{RUN_UNDERLINE:wdxConstants.FM_UL_WAVY_HEAVY});
test.outText("superscript",{RUN_SCRIPT:wdxConstants.FM_SCR_SUPERSCRIPT});
test.outLine();
test.outText("All letters are capitals ",{RUN_CAPS:true});
test.outText("subscript",{RUN_SCRIPT:wdxConstants.FM_SCR_SUBSCRIPT});
test.outLine();
test.outText("All letters are small capitals",{RUN_SMALL_CAPS:true});
test.outLine();
test.outText("Striked text",{RUN_STRIKE:true});
test.outLine();
test.outText("Highlighted text",{RUN_HIGHLIGHT:wdxConstants.FM_HL_CYAN});
test.outLine();
...
test.outFooter();
test.outLine({PAR_ALIGN:wdxConstants.FM_ALIGN_RIGHT});
test.outField(wdxConstants.FLD_PAGE,{RUN_UNDERLINE:wdxConstants.FM_UL_SINGLE});
test.endFooter();
test.outLine();
...
test.outLine();
var bm1 = test.addBookmarkStart("Bookmark name");       //adding bookmark for example of internal hyperlink in the next section
test.outText("This section demonstrates tables.");
test.addBookmarkEnd();
test.outLine();
test.outText("This section doesn't have header and footer.");
test.outTable({TBL_LAYOUT:wdxConstants.TBL_LAYOUT_AUTOFIT,TBL_WIDTH:{TBL_S_TYPE:wdxConstants.FM_TBLW_PCT,TBL_S_WIDTH:5000}});
test.outRow();
test.outCell(cellStyle);
test.outLine();
test.outText("Colored text in cell",{RUN_COLOR:"AB1321"});
test.outCell(cellStyle).setStyle({TBL_CELL_SHADING:{TBL_SHADING_FILL:"AAAAAA"},TBL_CELL_WIDTH:{TBL_S_TYPE:wdxConstants.FM_TBLW_PCT,TBL_S_WIDTH:2500}});
test.outLine();
...
test.outLine();
test.outHyperlink("Hyperlink to the bookmark of the previous section",bm1.item.getName());
test.outLine();
...
test.createStyle({STYLE_TYPE:wdxConstants.STYLE_TYPE_PAR,STYLE_NAME:"Custom style by wdxLibrary",STYLE_BASED_ON:"Base",STYLE_RUN:{RUN_BOLD:true,RUN_UNDERLINE:wdxConstants.FM_UL_WAVY_HEAVY},STYLE_PAR:{PAR_IND_LEFT:500}});
